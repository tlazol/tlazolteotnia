...
export function assertPaintImages(images: readonly string[], directory: string) {
  if (images.length === 0) throw new Error(`No paint images found in ${directory}`)
}
...
async function collectPaintImages(directory: string): Promise<string[]> {
  ...
  const files = await Promise.all(
    entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) return collectPaintImages(entryPath)
...
function paintImagesPlugin(): Plugin {
  ...
  return {
    ...
    async load(id) {
      if (id !== resolvedVirtualPaintImagesModule) return undefined
