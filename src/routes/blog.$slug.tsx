...
import type { BlogPost as BlogPostData, BlogPostSummary } from '#/lib/blog-post'
...
export const Route = createFileRoute('/blog/$slug')({
  loader: async ({ params }): Promise<PostPageData> => {
    return requireRouteData(await getPostData({ data: { slug: params.slug } }))
  },
...
function BlogPost() {
  const { post, posts, reactions }: PostPageData = Route.useLoaderData()
  const [currentReactions, setCurrentReactions] = useState(reactions)
  const accent = getPostAccent(post.slug)
  const navigate = useNavigate({ from: '/blog/$slug' })
  ...
  const topics: TopicChannel[] = [
    {
      ...
      count: posts.length,
      label: 'all-signals',
      onSelect: () => navigate({ to: '/', search: { topic: '' } })
    },
    ...getTagFilters(posts).map((tag) => ({
      active: false,
      count: tag.count,
      label: tag.name,
      onSelect: () => navigate({ to: '/', search: { topic: tag.name } })
    }))
  ]
