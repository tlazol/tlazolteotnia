...
export function PostModal({
  ...
  const [loadedReactions, setLoadedReactions] = useState<ReactionCount[] | null>(null)
  ...
  useEffect(() => {
    ...
    void fetch(`/api/reactions/${encodeURIComponent(summary.slug)}`, { signal: controller.signal })
      .then((response) => response.json() as Promise<{ reactions: ReactionCount[] }>)
      .then((result) => {
        setLoadedReactions(result.reactions)
        onReactions(summary.slug, result.reactions)
      })
      .catch((error) => {
        if (!(error instanceof DOMException && error.name === 'AbortError')) console.error(error)
      })
    return () => controller.abort()
  }, [onReactions, summary.slug])
          ...
          <article className="mx-auto w-full max-w-[720px]">
            <header className="relative border-b border-[color-mix(in_srgb,var(--post-accent)_18%,var(--line))] pt-9 pb-7 after:absolute after:right-0 after:bottom-[-1px] after:left-0 after:h-px after:opacity-65 after:content-[''] after:[background:var(--post-spectrum)] min-[680px]:pt-12">
              ...
              <ReactionBar
                ...
                reactions={loadedReactions ?? reactions}
                slug={summary.slug}
              />
              <p
