...
export function HomeTimeline({ posts, reactionsBySlug: initialReactions }: HomeTimelineProps) {
  ...
  function openPost(event: MouseEvent<HTMLAnchorElement>, post: BlogPostSummary) {
    ...
    setFetchedPost(null)
    void getModalPostData({ data: { slug: post.slug } }).then((result) => {
      if (result) {
        setFetchedPost(result.post)
        updateReactions(post.slug, result.reactions)
      }
    })
  }
  ...
  const topics: TopicChannel[] = [
    {
      ...
      count: posts.length,
      label: 'all-signals',
      onSelect: () => navigate({ search: { topic: '' } })
    },
    ...tags.map((tag) => ({
      active: selectedTag === tag.name,
      count: tag.count,
      label: tag.name,
      onSelect: () => navigate({ search: { topic: tag.name } })
    }))
  ]
